...
BEGIN
    FOR record_a IN (SELECT * FROM system_parameters.business_tables) LOOP
        ...
        create_sql := 'CREATE TABLE IF NOT EXISTS today.' || record_a.table_name || ' (';
        FOR field_record IN (SELECT * FROM system_parameters.business_fields WHERE table_id = record_a.table_id) LOOP
            ...
            IF field_record.is_foreign_key THEN
                ...
                FROM system_parameters.business_tables WHERE table_id = field_record.referenced_table_id; 
                SELECT field_name INTO field_name_var 
                FROM system_parameters.business_fields WHERE field_id = field_record.referenced_field_id; 
...
CREATE OR REPLACE FUNCTION system_parameters.create_unique_constraint() RETURNS VOID AS $$
DECLARE
    record_table RECORD;
    record_field RECORD;
...
BEGIN
    ...
    FOR record_table IN (
        ...
        archive_constraint_name := constraint_name || 'business_date';
        ...
        EXECUTE 'ALTER TABLE today.' || constraint_name || ' DROP CONSTRAINT IF EXISTS ' || constraint_name;
        EXECUTE 'ALTER TABLE today.' || constraint_name || ' ADD CONSTRAINT ' || constraint_name || ' UNIQUE (' || field_names || ');';
    END LOOP;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    FOR record_table IN (
        ...
        IF select_sql LIKE '% WHERE %' THEN
            -- delete
            delete from system_parameters.sqls where sql_name=record_a.table_name || '.SELECT_BY_UNIQUE';
